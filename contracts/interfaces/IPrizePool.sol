...
interface IPrizePool {
    /// @notice Deposits assets into the YieldSource in exchange for tokens.
    /// @dev Can only be called by the Lottery.
    /// @param to The address receiving the shares.
    /// @param amount The amount of assets to deposit.
    function depositTo(address to, uint256 amount) external;

    /// @notice Withdraws assets from the YieldSource.
    /// @dev Can only be called by the Lottery.
    ...
    /// @notice Claims interest accrued from deposits to the YieldSource.
